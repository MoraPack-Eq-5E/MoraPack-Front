/**
 * useContinuousSimulation
 * 
 * Hook para procesar y reproducir líneas de tiempo de simulación continua.
 * 
 * Características:
...
export function useContinuousSimulation({
  ...
  useEffect(() => {
    if (aeropuertos && aeropuertos.length > 0) {
      ...
      console.log(`[useContinuousSimulation] Almacenes inicializados: ${initialStorage.size} aeropuertos`);
    }
  }, [aeropuertos]);
  ...
  useEffect(() => {
    ...
    flightPairs.forEach(pair => {
      ...
      if (hasArrived && effectiveArrivalTime) {
        ...
        const legStart = departureTime;                 // hora de salida real/estimada
        const legEnd = effectiveArrivalTime;            // hora de llegada real/estimada
        const totalDuration = legEnd.getTime() - legStart.getTime();

        // Si el vuelo estaba ya en curso al inicio de la simulación, el progreso inicial
        // debe medirse desde simulationStartTime (no desde 0).
        // Esto evita que arranque en origen aunque esté IN_FLIGHT.
        const baseElapsed = (departureEvent.tipoEvento === 'IN_FLIGHT')
          ? (currentDateTime.getTime() - Math.max(simulationStartTime.getTime(), legStart.getTime()))
          : (currentDateTime.getTime() - legStart.getTime());

        const progress = Math.max(0, Math.min(1, baseElapsed/totalDuration)); // clamp 0–1
    ...
    if (flightsWithInvalidAirports > 0 && currentSimTime === 0) {
      console.warn(
        `[useContinuousSimulation] ${flightsWithInvalidAirports} vuelos tienen aeropuertos inválidos y serán omitidos`
      );
    ...
    if (shouldLog) {
      console.log(
        `[useContinuousSimulation] ${currentSimTime}s: ${activeFlightsList.length} activos, ${completedCount} completados, ${pendingCount} pendientes`
      );
    }
    ...
    if (currentSimTime === 0 && flightPairs.length > 0) {
      console.log(
        `[useContinuousSimulation] INICIO - Total pares de vuelo: ${flightPairs.length}`
      );
      console.log(
        `[useContinuousSimulation] Hora inicio simulación: ${simulationStartTime.toISOString()}`
      );
      console.log(
        `[useContinuousSimulation] Duración total: ${totalDurationSeconds / 60} minutos`
      );
    }
  ...
  useEffect(() => {
    if (timeline && flightPairs.length > 0) {
      console.log('[useContinuousSimulation] Inicializado:', {
